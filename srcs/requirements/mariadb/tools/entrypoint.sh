...
if [ ! -d /auth_pam_tool_dir/auth_pam_tool ]; then 
	...
	sed -i '/\[mariadb\]/a mysqld.socket=\/var\/run\/mysqld\/mysqld.sock' /etc/my.cnf.d/mariadb-server.cnf
	...
	mariadbd-safe & sleep 1 > /dev/null \
		&& mysql -u root -e "ALTER USER 'root'@'localhost' IDENTIFIED BY '$MRIADB_ROOT_PASSWORD';" \
